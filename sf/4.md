...
浮动是在当前行上向左或向右移动框（盒子）。浮动最有趣的特性是内容可能沿着它的侧边流动。内容沿着左浮动框的右侧向下流动，并沿着右浮动框的左侧向下流动 。

img

在 Web 中，HTML 文档会受到一些规则约束，特别是正常的文档流。在正常的文档流中，每个块级元素（比如 div、p等）垂直地堆叠在一起，从视图的顶部向下堆叠。但元素要是使用了浮动特性，元素会脱离文档流。
...
inline-start ：元素必须浮动在其容器的开始一侧，具体是哪一侧，将根据书写模式或者 direction (或 HTML 的 dir) 的值来决定，如果取值 ltr 是左侧，如果取值 rtl是右侧；
inline-end ：元素必须浮动在其容器的结束一侧，和 inline-start 类似，如果取值 ltr 是右侧，取值rtl 是左侧 。
假设我们有一段这样的 HTML 结构：
...
使用块级格式化上下文（BFC）来清除浮动，比如在浮动父元素上使用 float、overflow、display等；
借助伪元素 ::before 和 ::after ，在该元素上添加 clear:both 样式 ；
display 中最新属性之一 flow-root用来清除浮动。
来看看这三种清除浮动的示例代码：
...
庆幸的是，CSS 中提供了一个新模块，即 CSS Exclusions 模块，它致力于解决文本围绕图片方式 。它不需要依赖浮动 float ，也不管是否设置了 position 的值为 absolute、relative 或者 fixed 。允许内容围绕一个内联元素。如下图所示：
...
不幸运的是，到目前为止，该模块中提供的属性还只是一个实验性属性，目前只有在微软平台的浏览器中可以查看到相应的效果：
...
比如，@Andy Clarke 在他的文章《Art Direction For The Web Using CSS Shapes》中，使用 CSS Shapes 给 Web 设计带来很多边界性的突破，可以说它已超越了基本的 CSS Shapes，并向大家展示了如何使用它们给 Web 布局带来的突破。

img

上图来自于 @Andy Clarker 的《Art Direction For The Web Using CSS Shapes》一文！

也就是说，我们现在除了使用 CSS Shapes 的基本特性，可以构建文本围绕着圆形（circle() ）、椭圆（ellipse()）、矩形（inset()）和多边形（polygon()）流动之外，还可以利用 CSS 的一些其他特性，比如剪切（clip-path）和蒙板（mask）等，使 CSS Shapes 更超常的发挥，构建出更复杂和令人惊叹的布局，比如 V字型布局 、Z字型布局 、曲线布局 、对角线形状布局 、旋转布局 和任意图像定义的布局等。
...
不过，后面 CSS Shapes 模块又将 shape-inside 和 shape-outside 拆分出来放在不同的版本中。要是你查阅相关规范的话，你可以在 CSS Shapes Module Level 1 中查到 shape-outside 、shape-image-threshold 和 shape-margin 三个属性，它们分别用来：
...
如果你要查阅 shape-inside 就需要在 CSS Shapes Module Level 2 中才能索引到。在该模块中，你除了可以找到 shape-inside 属性之外，你还可以找到 shape-padding 和 shape() 函数。另外，shape-inside 还被扩展出一个新值 display ，具有此值的元素将使其内容(或包含的元素)自动沿显示边界对齐。
...
只不过，shape-inside 新扩展出来的 display 值相关的介绍，主要在 CSS Round Display Level 1 模中进行阐述。而且这部分相关的阐述已超出本文要阐述的范围，因此接下来不会做过多阐述。

W3C 的 CSS 工作小组把 shape-inside 、shape-padding 和 shape() 单独放到 Level2 中阐述，主要是到目前为止，还没有得到任何主流浏览器的支持。因此，这几个特性也被称为未来的 CSS Shapes 。

虽然 Level 2 没得到主流浏览器支持，但就目前来看，主流浏览器对 CSS Shapes Module Level 1 的支持度还是很好的：
...
定义基本形状
这里所说的基本形状是指可以让 Web 开发者生成更抽象的几何化 Web 布局，而不仅仅是简单的矩形和正方形 。换句话说，就是使用 CSS Shapes 中的 shape-outside 绘制的基本形状。简单地说，你可以在一个元素上运用 shape-outside 属性指定的 <basic-shape> 值，为元素应用形状。
...
你除了可以使用任何长度单位 <length>（比如，px、em、pt 、rem等）和百分比单位指定圆的半径之外，还也可以使用关键词 closest-side 和 farthest-side 来指定圆的半径：
...
再回过头来看 circle() 函数的半径 r 取百分比的情况。前面说过，如果半径的值是个百分比值，它是从参考框的宽度和高度解析出来的。它的计算稍微比较复杂，不过，我们可以按照下面这个公式来计算：
...
通过 url() 函数引用带有透明度的图片。可能会令大家感到意外，即使这么做了，效果依旧。这里有一个细节需要注意，url() 引入的图片必须是同域的（要解决 图片 CROS 兼容性），否则 shape-outside 会被视为是 none 。所以你看到的效果和没有设置 shape-outside 是一样的。
...
需要注意的是，mask-image 和 shape-image-threshold 一样，引入的图片必须是同域的（要解决图片 CROS 兼容性），建议你直接使用 URI，这样易于避免图片 CROS 的兼容性问题。
...
shape-margin 创建与多边形形状shape-outside 的偏移量。红色区域显示 200 x 200px 的浮动形状，蓝色区域显示 10px 的偏移量（shape-margin 的值）。
...
这个效果和我们使用常规 margin 属性时的效果类似，但是 shape-margin 只影响 shape-outside 值周围的空间。只有在坐标系统中有空间时，它才会在形状周围添加间距。这就是为什么在上面的例子中，圆半径被设置为 40%，而不是 50%。如果半径设置为 50%，圆将占据坐标系统中的所有空间，因此没有 shape-margin的效果。
...
除了浏览器内置的 CSS Shapes 形状编辑器之外，还可以借助 Clippy 工具来编辑复杂的形状：

img

Clippy 工具地址：bennettfeely.com/clippy/
...
综合示例
文章开头提到 @Andy Clarke 在他的文章《Art Direction For The Web Using CSS Shapes》向大家展示了很多有意思的不规则 Web 布局。比如 V字型布局 、Z字型布局 、曲线布局 、对角线形状布局 、旋转布局 和任意图像定义的布局 等。我们就先以 V 字型布局为例。
...
如此一来，可以借助 ::before 和 ::after 来使用两张图片，不过对于类似这样的三角形，还可以借助polygon() 来绘制。为了方便，可以使用 Clippy 工具来完成：
